...
public class UserService : IUserService
{
    ...
    public bool UpdateUser(User user)
    {
        var result =
            _service.Update<User>(
                "UPDATE public.user SET Id=@Id, name = @Name, email=@Email, password=@Password WHERE Id = @Id", user);
        return result;
    }
    ...
    {
        var result = _service.GetById<int>("SELECT COUNT(1) FROM public.user WHERE password = @Id",
            new { Id = password });
        return result == 1;
    }
